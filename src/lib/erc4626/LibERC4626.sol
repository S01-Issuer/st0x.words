...
library LibERC4626 {
    ...
    /// @return assetsFloat The equivalent amount of underlying assets, as a
    /// Rain Float re-encoded at the underlying asset token's decimals (not the
    /// share decimals the input was decoded at).
    function convertToAssets(IERC4626Minimal vault, Float sharesFloat) internal view returns (Float assetsFloat) {
    ...
    /// @return sharesFloat The equivalent number of vault shares, as a Rain
    /// Float re-encoded at the vault share token's decimals (not the asset
    /// decimals the input was decoded at).
    function convertToShares(IERC4626Minimal vault, Float assetsFloat) internal view returns (Float sharesFloat) {
        (uint8 shareDecimals, uint8 assetDecimals) = _vaultScales(vault);
